      ...
      {groups.map((g) => {
        ...
        return (
          <section key={g} className="flex flex-col gap-2">
            <h2 className="text-sm font-medium text-tg-hint">{GROUP_TITLE[g]}</h2>
            <div className="flex flex-col gap-2">
              {groupTasks.map((task) => (
                <button
                  key={task.id}
                  onClick={() => navigate(`/tasks/${task.id}`)}
                  className="flex flex-col gap-2 rounded-2xl bg-tg-secondaryBg p-4 text-left"
                >
                  ...
                  {task.deadline && (
                    <span className="text-xs text-tg-hint">
                      Deadline: {new Date(task.deadline).toLocaleString("uz-UZ")}
                    </span>
                  )}
                </button>
              ))}
